...
class MainWindow(QtWidgets.QMainWindow,): #class made for creating the window and everything that needs to be done in it
    ...
    def __init__(self, *args, **kwargs): #constructor of the class
        ...
        pen = pg.mkPen(color=(0, 157, 224)) #setting the line color to the ist blue
        
        self.data_line =  self.graphWidget.plot(self.x, self.y, pen=pen) #initializing line
        self.timer = QtCore.QTimer() #initilizing timer
        self.timer.setInterval(1)
        self.timer.timeout.connect(self.update_plot_data) #while timer is connected, the update_plot_data function repeats
    ...
    def initialQuestion(self):
        ...
        ser.write(self.option.encode('utf-8')) #writes the option to serial, which arduino will read
        arduinoResponse = ser.readline().decode('utf-8').rstrip() #gets the arduino response
        while (arduinoResponse.isdigit() == False):
